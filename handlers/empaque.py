...
async def menu_empaque(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message:
        await update.message.reply_text("🔑 Ingrese la clave de acceso para Empaque:")
    else:
        await update.callback_query.message.edit_text("🔑 Ingrese la clave de acceso para Empaque:")
    return ESPERANDO_CLAVE

async def recibir_clave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not validar_clave("empaque", update.message.text.strip()):
...
async def recibir_caja(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    d = context.user_data
    resumen = (
        f"**Confirme el Registro de Empaque:**\n"
        f"Fecha: {d['fecha']}\n"
        f"Empresa: {d['empresa']}\n"
        f"Tipo: {d['tipo']}\n"
    ...
    kb = [
        ...
        [InlineKeyboardButton("✏️ Editar", callback_data="editar_carga_empaque")], 
...
async def confirmar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "editar_carga_empaque": 
        await q.edit_message_text("✏️ Editando la carga de Empaque. Ingrese la fecha (DD/MM/AAAA):")
        return ESPERANDO_FECHA
...
def build_empaque_handler():
    return ConversationHandler(
        entry_points=[CommandHandler("empaque", menu_empaque)],
        states={
